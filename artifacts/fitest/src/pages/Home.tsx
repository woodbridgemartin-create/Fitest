...
function PrintReport({ email, auditPath, questions, answers, score }: ReportProps) {
  ...
  return (
    <div className="print-report hidden print:block">
      ...
      <h2 className="print-audit-title">YOUR RESULT : {auditLabel.toUpperCase()}</h2>
      <div className="print-score-block">
        <div className="print-score">{score}/100</div>
        <div className="print-tier-label">
          TIER : {tier.name.toUpperCase()} ({tier.min} TO {tier.max})
        </div>
      </div>
      <div className="print-tier-bar">
        {TIERS.map((t) => (
          <span key={t.name} className={t.name === tier.name ? "print-tier-active" : ""}>
            {t.name.toUpperCase()}
          </span>
        ))}
      </div>
      ...
      <div className="print-questions">
        {questions.map((q, i) => (
          <div key={i} className="print-question-row">
            ...
            <span className="print-q-score">{answers[i]}/5</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Home() {
  ...
  const handleStartAudit = (path: AuditPath) => {
    ...
    setEmailError("");
    setAnswers(
      Array(path === "gym" ? GYM_QUESTIONS.length : BUSINESS_QUESTIONS.length).fill(3)
    );
    setTimeout(() => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background text-foreground selection:bg-primary/30 print:hidden">
        <main className="container mx-auto px-4 py-16 max-w-5xl">
          ...
          <section className="mb-20">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {TIERS.map((tier, index) => (
                ...
                >
                  <Card
                    className={`bg-card/50 border ${tier.border} ${
                      activeTier?.name === tier.name ? tier.bg : ""
                  ...
                  {[
                    {
                      ...
                      desc: "Workforce performance audit. Energy, recovery, stress and output for teams.",
                    },
                    {
                      path: "gym" as AuditPath,
                      letter: "G",
                      title: "For Gyms",
                      desc: "Member performance audit. Training, recovery, nutrition and physical resilience.",
                    },
                  ].map(({ path, letter, title, desc }) => (
                    <motion.button
                      key={path}
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => handleStartAudit(path)}
                      className="group bg-card border border-card-border hover:border-primary/40 rounded-2xl p-8 text-left transition-all duration-200 hover:bg-card/80 cursor-pointer"
                    >
          ...
          {/* Audit Questions */}
          <AnimatePresence>
            {isAuditStarted && !showEmailGate && resultScore === null && (
              <motion.section
                ref={auditRef}
                    ...
                    >
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex gap-3 items-start">
                          <span className="text-xs font-mono text-muted-foreground/50 w-5 shrink-0 mt-1">
          ...
          {/* Email Gate */}
          <AnimatePresence>
            {showEmailGate && resultScore === null && (
              <motion.section
                ref={emailRef}
                ...
                className="max-w-md mx-auto text-center mb-24"
              >
                <div className="bg-card border border-card-border rounded-2xl p-10 shadow-2xl">
            ...
            {resultScore !== null && activeTier && (
              ...
              >
                    ...
                    {TIERS.map((t) => (
                      <span
                        ...
                        className={`text-xs font-bold uppercase tracking-wider ${
                          activeTier.name === t.name ? t.color : "text-muted-foreground/40"
                        }`}
                      >
                        {t.name}
                      </span>
                ...
                <motion.div
                  ...
                  className="bg-card border border-card-border rounded-xl p-6 text-left"
                >
                  ...
                  <div className="space-y-3">
                    {questions.map((q, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <span className="text-xs font-mono text-muted-foreground/50 w-5 shrink-0 mt-0.5">
                          {i + 1}.
                        </span>
                ...
                >
                  ...
                  <Button
                    ...
                    size="lg"
                    onClick={handleRetake}
                    className="h-12 px-8 border-border hover:bg-muted font-semibold uppercase tracking-wider"
                  >
                    Retake Audit
                  </Button>
                </motion.div>
